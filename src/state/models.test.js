...
test("an NVFP4 load keeps the source label and names the load format", () => {
  assert.equal(modelWeightLabel({ format: "hf-safetensors", quantize: "nvfp4" }), "Safetensors · NVFP4 load");
  assert.equal(modelSubtitle({ format: "hf-safetensors", quantize: "nvfp4", sourceBits: 16 }), "Safetensors · NVFP4 load");
});
